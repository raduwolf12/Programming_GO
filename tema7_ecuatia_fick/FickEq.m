...
time = zeros(maxk);
sustime = zeros(n+1,maxk);
inftime = zeros(n+1,maxk);
...
for k = 1:maxk % begin time steps
    ...
    for m =1:20
        for i = 1:nn %compute Jacobian matrix
            ...
            if i==n+2
                F(i) = inf(1) - infp(1) + b*dt*inf(1) -alpha*2*(-inf(1) + inf(2)) - a*dt*sus(1)*inf(1);
            ...
            if i>n+2 && i<nn
                i_shift = i - (n+1);
                F(i) = inf(i_shift) - infp(i_shift) + b*dt*inf(i_shift) - alpha*(inf(i_shift-1) - 2*inf(i_shift) + inf(i_shift+1)) - a*dt*sus(i_shift)*inf(i_shift);
            ...
            if i==nn
                F(i) = inf(n+1) - infp(n+1) + b*dt*inf(n+1) - alpha*2*(-inf(n+1) + inf(n)) - a*dt*sus(n+1)*inf(n+1);
