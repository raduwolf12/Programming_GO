...
func main() {
	...
	for i := 1; i < p; i++ {
		...
		fmt.Fprintf(w, "f %d %d %d \n", i, i+1, 2*p+i+1)
		a = computeDistance(vertex[i-1], vertex[i])
		b = computeDistance(vertex[i], vertex[2*p+i])
		area += computeArea(a, b, c)

		fmt.Fprintf(w, "f %d %d %d \n", p+i, 3*p+i, 3*p+i+1)
		a = computeDistance(vertex[p+i-1], vertex[3*p+i-1])
		b = computeDistance(vertex[3*p+i-1], vertex[3*p+i])
